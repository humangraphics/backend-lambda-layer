 ...
 *
 * <p>
 * 1. loads the user's handler. <br/>
 * 2. enters the Lambda runtime loop which handles function invocations as defined in the Lambda
 * Custom Runtime API.
 *
 * <p>
...
public class AWSLambda {
  ...
  private static final String NETWORKADDRESS_CACHE_NEGATIVE_TTL_ENV_VAR =
      "AWS_LAMBDA_JAVA_NETWORKADDRESS_CACHE_NEGATIVE_TTL";

  private static final String NETWORKADDRESS_CACHE_NEGATIVE_TTL_PROPERTY =
  ...
  static {
    ...
    // The ca-certificates package provides /etc/pki/java/cacerts which becomes the symlink
    // destination
    // of $java_home/lib/security/cacerts when java is installed in the chroot. Given that java is
    // provided
  ...
  private static LambdaRequestHandler findRequestHandler(final String handlerString,
    ...
    try {
      ...
      throw makeInitErrorUserFault(t, HandlerInfo.className(handlerString));
    }

    final LambdaRequestHandler requestHandler = EventHandlerLoader.loadEventHandler(handlerInfo);
    // if loading the handler failed and the failure is fatal (for e.g. the constructor threw an
    // exception)
